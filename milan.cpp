...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < 4; i++) /// scanning partitions in the physical disk
    {
        ...
        switch (PartitionTbl->chType) {
            case PART_DOS2_FAT:
                strcpy(szTmpStr, "FAT12\0");
                break;
            case PART_DOSX13:
            case PART_DOS4_FAT:
            case PART_DOS3_FAT:
                strcpy(szTmpStr, "FAT16\0");
                break;
            case PART_DOS32X:
            case PART_DOS32:
                strcpy(szTmpStr, "FAT32\0"); //Normal FAT32
                break;
            case 7:
                strcpy(szTmpStr, "NTFS\0"); // NTFS , v r interested only on this
                QuickCheck(hDrive, stDrive.dwNTRelativeSector, 512);
                i = 6;
                break;
            case PART_LINUX_SWAP:
                strcpy(szTmpStr, "Linux Swap\0"); // NTFS , v r interested only on this
                break;
            case PART_LINUX:
                strcpy(szTmpStr, "Linux Partition\0"); // NTFS , v r interested only on this
                break;
            default:
                strcpy(szTmpStr, "Unknown\0");
                break;
        }
    ...
    printf("Pocelo skeniranje fajlova \n ");
    for (i = 0; (i < 0xFFFFFFFF); i++) {

        memset(szBuffer, '\0', 300);
        memset(&stFInfo, '0', sizeof(CNTFSDrive::ST_FILEINFO));

        nRet = m_cNTFS.GetFileDetail(i + 30, stFInfo); // get the file detail one by one - the +30 offset is to avoid inital records in the filesystem
        
        
        if ((nRet == 123) || (nRet == ERROR_INVALID_PARAMETER))
        {
            printf("wtf\n");
            exit(0);
        }
        ...
        if (stFInfo.bDeleted)
            strcat(szBuffer, "Deleted-"); // if it is deleted
        
        
        // strcpy(szBuffer,"");
//        if (stFInfo.bDeleted && stFInfo.szFilename) //&& strncmp(stFInfo.szFilename, "0000000000000000000000000000000000000000000000000", 50))
//        {
            //dwDeleted++;
//            strcpy(szBuffer, "Yes");
            if (i == 299143)
